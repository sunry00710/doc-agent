export type PromotionFinding = { category: string; summary: string; evidence: string; requirement_id: string | null; severity: string; mandatory: boolean; blocking: boolean; human_review_required: boolean }
export type ApiError = { code: string; message: string; request_id: string; retryable: boolean; details?: { findings?: PromotionFinding[] } }
export type User = { id: string; username: string; role: 'user' | 'reviewer' | 'admin'; is_active: boolean }
export type ProjectMember = { user_id: string; project_id: string; membership_role: 'contributor' | 'reviewer' | 'owner' }
...
export type ContractAssessment = { requirement_id: string; status: 'satisfied' | 'unsatisfied' | 'unknown'; evidence: string; mandatory: boolean; blocking: boolean }
export type SupervisorConcern = { id: string; category: string; summary: string; requirement_id: string | null; assigned_to: string; evidence: string; mandatory: boolean; blocking: boolean }
...
export class ApiClient {
  ...
  chat(token: string, request: { text: string; confirmed?: boolean; idempotency_key?: string; project_id?: string; document_version_id?: string; knowledge_space_ids?: string[] }): Promise<ChatResponse> { return this.request('/chat', { method: 'POST', token, body: request }) }
  ...
  projectMembers(token: string, projectId: string): Promise<ProjectMember[]> { return this.request(`/projects/${projectId}/members`, { token }) }
  addProjectMember(token: string, projectId: string, userId: string, membershipRole: ProjectMember['membership_role']): Promise<ProjectMember> { return this.request(`/projects/${projectId}/members`, { method: 'POST', token, body: { user_id: userId, membership_role: membershipRole } }) }
  ...
  requestChanges(token: string, reviewId: string, text: string, source_range: { start: number; end: number }, expected_revision: number): Promise<Review> { return this.request(`/reviews/${reviewId}/request-changes`, { method: 'POST', token, body: { text, source_range, expected_revision } }) }
  ...
  createPromotion(token: string, body: { version_id: string; target_space_id: string }): Promise<Promotion> { return this.request('/knowledge/promotions', { method: 'POST', token, body }) }
  ...
  private async request<T>(path: string, options: RequestOptions & { token?: string } = {}): Promise<T> {
    ...
    if (!response.ok) {
      const apiError = await this.error(response)
      this.handleAuthExpiry(apiError)
      throw apiError
    }
    return await response.json() as T
  }
  // token 过期（401）：清理本地凭据并通知 App 回登录页，避免用户在各页面看到"加载失败"却不知原因
  private handleAuthExpiry(apiError: ApiError) {
    if (apiError.code !== 'authentication_error' && apiError.code !== 'unauthorized') return
    if (typeof sessionStorage === 'undefined') return
    sessionStorage.removeItem('doc-agent-token')
    window.dispatchEvent(new CustomEvent('doc-agent-auth-expired'))
  }
