...
export type Promotion = { id: string; version_id: string; target_space_id: string; requested_by: string; reviewed_by: string | null; status: string; quality_status: string; findings: Record<string, unknown>[]; policy_version: string; authority_level: number; public_authority: boolean }
...
export class ApiClient {
  ...
  chat(token: string, request: { text: string; confirmed?: boolean; idempotency_key?: string }): Promise<ChatResponse> { return this.request('/chat', { method: 'POST', token, body: request }) }
  ...
  private async request<T>(path: string, options: RequestOptions & { token?: string } = {}): Promise<T> {
    ...
    const response = await fetch(`${this.baseUrl}${path}`, { ...init, headers: { ...(body instanceof URLSearchParams ? {} : body === undefined ? {} : { 'Content-Type': 'application/json' }), ...(token ? { Authorization: `Bearer ${token}` } : {}), ...headers }, body: body instanceof URLSearchParams ? body : body === undefined ? undefined : JSON.stringify(body) })
    if (!response.ok) throw await this.error(response)
