...
> 前序文档：`docs/交接记录-2026-09-11上午.md`（上午验收）、`-final.md`（下午改造）、
...
| 交付 | 文档 | `README.md`（60 秒启动/命令/目录地图）、`docs/使用手册.md`（三角色操作手册）、`docs/架构说明.md`（架构与关键机制）、`docs/部署指南.md`（服务器/内网部署：systemd/nginx/升级/备份/安全清单——交付检查时补充） |
...
- **迁移蓝图**：`docs/多角色迁移方案.md` —— user_roles 表、双读过渡、
  接口 `roles: list`、JWT 升级、前端多选，含风险表与验收清单。下一轮按该文档实施。

## 七、交付后修复：E2E 偶发失败（2026-09-14）

### 7.1 现象

交付基线复跑时 `npm --prefix web run test:e2e` 不再稳定：多次运行出现
**10 passed / 1 failed**，失败固定在 `review-workspace.spec.ts`，
作者点「提交 v1 评审」后按钮停在「提交中…」，断言等 5s 未见「已提交」。
...
[SQL: UPDATE document_reviews SET reviewer_id=?, workflow_revision=? WHERE ... AND workflow_revision = ?]
```

API 侧写路径普遍是「先 SELECT 校验、再 UPDATE」：在 WAL 模式下，这样的**延迟事务**
一旦在读写之间有别的连接（独立 worker 建索引）提交，SQLite 会立刻抛
`SQLITE_BUSY_SNAPSHOT`——**`busy_timeout` 对这种「写升级」冲突无效**，
于是表现为随机 500 / 界面卡在提交中。此前 11 passed 属时序运气。

### 7.3 修复

| 文件 | 改动 |
|---|---|
| `app/db/session.py` | 新增 `immediate_transactions` 开关：开启时关闭 pysqlite 隐式 BEGIN，并在 `begin` 事件发 `BEGIN IMMEDIATE`；`busy_timeout` 5000→15000 |
| `app/main.py` | API 引擎以 `immediate_transactions=True` 创建（worker 侧保持延迟事务 + 既有重试） |
| `web/e2e/review-workspace.spec.ts` | 断言窗口 5s→15s：写操作与索引 worker 抢锁时会排队，避免把「等待」误判为失败 |
| `docs/开发指南.md` | §五 记录该取舍，防止后续被「优化」回去 |

### 7.4 验证证据（2026-09-14 复跑）

| 项 | 结果 |
|---|---|
| 后端 pytest | **227 passed, 1 skipped**（26.1s） |
| 前端 vitest | **40 passed / 8 文件**（29.1s） |
| E2E（单 spec） | `review-workspace.spec.ts` **连续 3 次通过**，日志 0 处 `database is locked` |
| E2E（全量） | **11 passed**，连续 2 次，日志 0 处 `database is locked` |
| ruff / tsc | `All checks passed!` / 干净 |

> 本次修复停留在工作区，**未 commit、未 push**；`main` 与
> `feature/doc-agent-foundation` 仍在 `65eff50`。
