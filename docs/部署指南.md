...
> 不适用：公网生产、多实例高并发、PostgreSQL（替换路径见 `docs/架构说明.md` §六）。
...
| `MODEL_PROVIDER` | `self` | **不设置则默认 fake（离线演示回复）** |
| `SELF_AI_*` | 按实际模型服务填写 | endpoint / api_key / model 三项缺一即启动报错 |
| `BACKEND_HOST` | `127.0.0.1` | 由 nginx 反代时保持本机；直连暴露再改 `0.0.0.0` |
| `CORS_ALLOWED_ORIGINS` | 仅跨域接入方需要 | JSON 数组，如 `["https://app.example.com"]`；同源经 nginx 时留空 |
...
下载失败会让检索/索引**抛错而非降级**（见 `docs/开发指南.md` §五"语义检索硬失败"）。
...
server {
    listen 80;
server_name doc-agent.example.com;       # 按实际域名/主机名改
...
- [ ] 模型接入使用受控端点，敏感材料不出公网（`MODEL_PROVIDER` 非 `fake` 时确认端点归属）
- [ ] 备份任务在跑且演练过一次恢复
